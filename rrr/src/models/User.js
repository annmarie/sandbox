  ...
  save() {
    ...
    const insert = () => {
      ...
      const query = "INSERT INTO " +
        " user (email, password_digest, admin) " +
        " VALUES (?, ?, ?)"
      const qargs = [this.email, this.password_digest, this.admin, this.id]

      return db.query(query, qargs).then(rows => {
    ...
    const update = (user) => {
      ...
      const query = "UPDATE user " +
        ...
        " WHERE id=? "
      const qargs = [email, admin, pswd, this.id]
