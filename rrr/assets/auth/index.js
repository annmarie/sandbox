...
class Auth {
  ...
  constructor($authDisplay, $authModals) {
    this.$isAuthDisplay = $authDisplay.find('[data-target=is-auth-display]')
    this.$notAuthDisplay = $authDisplay.find('[data-target=not-auth-display]')
    this.$loginModalBox = $authModals.find('[data-target=login-modal]')
    this.$loginOpenModal = $authDisplay.find('[data-action=login-modal-open]')
